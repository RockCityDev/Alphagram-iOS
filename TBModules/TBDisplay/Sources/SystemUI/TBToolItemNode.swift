...
public struct TBToolItem<T> {
    public let backgrounndColor: UIColor?
    public let iconName: String
    public let title: String
    public let url: String?
    public let type: T
    
    public init(type: T,title: String, iconName: String , color: UIColor?, url: String? = nil) {
        self.type = type
        self.title = title
...
public struct TBItemNodeTheme {
    ...
    public init(itemsize: CGSize = CGSize(width: 60, height: 60),
...
final public class TBToolItemNode<T>: ASDisplayNode {
    
    private let backgroundNode: ASDisplayNode
    private let officialNode: ASImageNode
    private let imageNode: UIImageView
    ...
    public var clickEvent: ((T) -> Void)?
    
    public init(theme: PresentationTheme? = nil, itemTheme: TBItemNodeTheme = TBItemNodeTheme(), isOfficial: Bool = false) {
    ...
    override public func didLoad() {
        ...
        self.backgroundNode.view.addSubview(self.imageNode)
    ...
    public func updateNodeBy(_ item: TBToolItem<T>) {
        self.toolItem = item
        self.backgroundNode.backgroundColor = item.backgrounndColor
        if item.iconName.hasPrefix("http") {
            self.imageNode.sd_setImage(with: URL(string: item.iconName), placeholderImage: nil)
        } else {
    ...
    public func updateBadge(_ count: Int, unreadHasUnmuted: Bool = false, selectionFraction: CGFloat = 1.0) {
