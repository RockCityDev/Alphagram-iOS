...
public enum TBItemListDisclosureItemTitleColor {
    case primary
    case accent
}

public enum TBItemListDisclosureStyle {
    case arrow
    case none
}

public enum TBItemListDisclosureLabelStyle {
    case text
    case detailText
    ...
    case textLeftIcon(TBSDImage, CGSize, UIColor) 
}

public class TBItemListDisclosureItem: ListViewItem, ItemListItem {
    let presentationData: ItemListPresentationData
    let icon: UIImage?
    let title: String
    let titleColor: TBItemListDisclosureItemTitleColor
    let enabled: Bool
    let label: String
    let labelStyle: TBItemListDisclosureLabelStyle
    public let sectionId: ItemListSectionId
    let style: ItemListStyle
    let disclosureStyle: TBItemListDisclosureStyle
    let action: (() -> Void)?
    let clearHighlightAutomatically: Bool
    public let tag: ItemListItemTag?
    public let shimmeringIndex: Int?
    
    public init(presentationData: ItemListPresentationData, icon: UIImage? = nil, title: String, enabled: Bool = true, titleColor: TBItemListDisclosureItemTitleColor = .primary, label: String, labelStyle: TBItemListDisclosureLabelStyle = .text, sectionId: ItemListSectionId, style: ItemListStyle, disclosureStyle: TBItemListDisclosureStyle = .arrow, action: (() -> Void)?, clearHighlightAutomatically: Bool = true, tag: ItemListItemTag? = nil, shimmeringIndex: Int? = nil) {
    ...
    public func nodeConfiguredForParams(async: @escaping (@escaping () -> Void) -> Void, params: ListViewItemLayoutParams, synchronousLoads: Bool, previousItem: ListViewItem?, nextItem: ListViewItem?, completion: @escaping (ListViewItemNode, @escaping () -> (Signal<Void, NoError>?, (ListViewItemApply) -> Void)) -> Void) {
        async {
            let node = TBItemListDisclosureItemNode()
    ...
    public func updateNode(async: @escaping (@escaping () -> Void) -> Void, node: @escaping () -> ListViewItemNode, params: ListViewItemLayoutParams, previousItem: ListViewItem?, nextItem: ListViewItem?, animation: ListViewItemUpdateAnimation, completion: @escaping (ListViewItemNodeLayout, @escaping (ListViewItemApply) -> Void) -> Void) {
        Queue.mainQueue().async {
            if let nodeValue = node() as? TBItemListDisclosureItemNode {
...
public class TBItemListDisclosureItemNode: ListViewItemNode, ItemListItemNode {
    ...
    let labelImageNode: TBSDImageNode
    
    private let activateArea: AccessibilityAreaNode
    
    private var item: TBItemListDisclosureItem?
    ...
    public init() {
        ...
        self.labelImageNode = TBSDImageNode()
    ...
    public func asyncLayout() -> (_ item: TBItemListDisclosureItem, _ params: ListViewItemLayoutParams, _ insets: ItemListNeighbors) -> (ListViewItemNodeLayout, () -> Void) {
        ...
        return { item, params, neighbors in
            ...
            var updatedLabelImage: TBSDImage?
            ...
            if case let .color(color) = item.labelStyle {
                ...
                if updatedColor {
                    updatedLabelImage = .image(generateFilledCircleImage(diameter: 17.0, color: color))
                }
            }
            if case let .textWithIcon(image) = item.labelStyle {
                updatedLabelImage = .image(generateTintedImage(image: image, color: item.presentationData.theme.list.itemSecondaryTextColor))
            }else if case let  .textLeftIcon(icon, _, _) = item.labelStyle {
                updatedLabelImage = icon
            }else if case let .image(image, _) = item.labelStyle {
                updatedLabelImage = .image(image)
            }else if case let .textLeftIcon(image, _, _) = item.labelStyle{
            ...
            case let .coloredText(color):
                ...
                labelFont = titleFont
            case let .textLeftIcon(_, _, labelTextColor):
                labelBadgeColor = labelTextColor
                labelFont = titleFont
            default:
                labelBadgeColor = item.presentationData.theme.list.itemSecondaryTextColor
                labelFont = titleFont
            ...
            return (ListViewItemNodeLayout(contentSize: contentSize, insets: insets), { [weak self] in
                if let strongSelf = self {
                    ...
                    } else if case let .textLeftIcon(_, size, _) = item.labelStyle {
                        if let updatedLabelImage = updatedLabelImage {
                            strongSelf.labelImageNode.image = updatedLabelImage
                        }
                        if strongSelf.labelImageNode.supernode == nil {
