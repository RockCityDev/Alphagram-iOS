...
public final class TBShareManager {
    ...
    func share(text: String, peerIds: [PeerId], topicIds: [PeerId: Int64], showNames: Bool, silently: Bool) -> Signal<ShareState, ShareControllerError>? {
        ...
        case let .url(url):
            for peerId in peerIds {
                ...
                shareSignals.append(enqueueMessages(account: self.currentAccount, peerId: peerId, messages: messages))
            }
        ...
        case let .messages(messages):
            for peerId in peerIds {
                ...
                shareSignals.append(enqueueMessages(account: self.currentAccount, peerId: peerId, messages: messagesToEnqueue))
            }
        case let .fromExternal(f):
            return f(peerIds, text, self.currentAccount, silently)
    ...
    func shareExternal(what: Bool) -> Signal<ShareExternalState, NoError>? {
        ...
        case let .messages(messages):
            ...
            loop: for media in message.media {
                ...
                let accountPeerId = self.currentAccount.peerId
                ...
                for attribute in message.attributes {
                    if let attribute = attribute as? RestrictedContentMessageAttribute {
                        restrictedText = attribute.platformText(platform: "ios", contentSettings: self.currentContext.currentContentSettings.with { $0 }) ?? ""
                    }
                }
        ...
        return (collectExternalShareItems(strings: self.presentationData.strings, dateTimeFormat: self.presentationData.dateTimeFormat, nameOrder: self.presentationData.nameDisplayOrder, engine: TelegramEngine(account: self.currentAccount), postbox: self.currentAccount.postbox, collectableItems: collectableItems, takeOne: !self.immediateExternalShare)
                |> deliverOnMainQueue) |> map { state in
            switch state {
            case .progress:
                return .preparing
            case let .done(items):
    ...
    private func saveToCameraRoll(representations: [ImageRepresentationWithReference]) {
        ...
        let _ = (SaveToCameraRoll.saveToCameraRoll(context: context, postbox: context.account.postbox, mediaReference: .standalone(media: media)) |> map(Optional.init)).start()
    }
    ...
    private func saveToCameraRoll(mediaReference: AnyMediaReference) {
        ...
        let _ = (SaveToCameraRoll.saveToCameraRoll(context: context, postbox: context.account.postbox, mediaReference: mediaReference) |> map(Optional.init)).start()
    }
