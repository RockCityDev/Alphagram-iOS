...
private struct State:Equatable {
    ...
    static func == (lhs: State, rhs: State) -> Bool {
        ...
        if lhs.walletCount != rhs.walletCount {
            return false
        }
        return true
    }
    ...
    func isValid() -> Bool {
        if self.errorMessage.isEmpty && !self.inputString.isEmpty {
            return true
        }
        return false
    }
...
public class TBCreatWalletNodeView: UIView {
    ...
    let params: TBCreatWalletController.Params
    weak var controller: TBCreatWalletController?
    ...
    init(context:AccountContext, controller:TBCreatWalletController,  params: TBCreatWalletController.Params) {
        self.context = context
        self.params = params
        self.controller = controller
        let initialState = State(inputString: "", errorMessage: "", walletCount: 0)
        ...
        let _ = TBMyWalletManager.shared.allWalletsSignal.start(next: { [weak self] allWallets in
            self?.updateState { current in
                var current = current
                current.walletCount = allWallets.count
                return current
            }
        })
    ...
    private func reloadWithState(_ state: State) {
        self.userInputView.updateWithInputString(state.inputString, errorMessage: state.errorMessage, placeHolder: "-\(state.walletCount + 1)")
        if state.isValid() {
...
extension TBCreatWalletNodeView: UITextFieldDelegate {
...
extension TBCreatWalletNodeView: UIScrollViewDelegate {
