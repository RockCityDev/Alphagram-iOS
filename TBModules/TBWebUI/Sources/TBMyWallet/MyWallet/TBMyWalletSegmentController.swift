...
final class TBMyWalletSegmentController: SegementSlideViewController {
    
    private let context: AccountContext
    private var presentationData: PresentationData
    
    private let segmentedView = TBJXSegmentedView()
    let myHeaderView: TBMyWalletHeaderView
    ...
    init(context: AccountContext, presentationData: PresentationData) {
        ...
        self.myHeaderView = TBMyWalletHeaderView(context: context, presentationData: presentationData)
        super.init(nibName: nil, bundle: nil)
        self.tokenVc.balanceUsdChange = { balanceUsd in
            self.myHeaderView.updateMoney("$" + balanceUsd + " USD")
        }
    }
    ...
    override func segementSlideHeaderView() -> UIView? {
        let myHeaderView = self.myHeaderView
        myHeaderView.translatesAutoresizingMaskIntoConstraints = false
        let height:CGFloat = 297+10
        myHeaderView.heightAnchor.constraint(equalToConstant: height).isActive = true
        return myHeaderView
    }
...
extension TBMyWalletSegmentController: SegementSlideSwitcherDataSource {
...
extension TBMyWalletSegmentController: JXSegmentedViewDelegate {
