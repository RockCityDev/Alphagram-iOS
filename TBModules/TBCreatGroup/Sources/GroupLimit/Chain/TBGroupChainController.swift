...
private struct TBItemLimitState: Equatable {
    ...
    var currentItem:TBWeb3ConfigEntry.Chain?
...
private struct TBItemLimitArguments {
    ...
    let selectItem:(TBWeb3ConfigEntry.Chain)->Void
    let done:()->Void
}
...
private enum TBItemLimitEntry: ItemListNodeEntry {
    ...
    case item(TBWeb3ConfigEntry.Chain, Int, Bool)
...
private func itemEntries(presentationData: PresentationData, state: TBItemLimitState) -> [TBItemLimitEntry] {
    ...
    let tokenList = state.config.chainType
...
public func chainListControllerImpl(context: AccountContext, config: TBWeb3ConfigEntry, selectItem: TBWeb3ConfigEntry.Chain? = nil, update:@escaping (TBWeb3ConfigEntry.Chain) -> Void, completion: ((PeerId, @escaping () -> Void) -> Void)? = nil) -> ViewController {
    
    let initialState = TBItemLimitState(config: config, currentItem: selectItem )
    ...
    |> map { presentationData, state -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        let controllerState = ItemListControllerState(presentationData: ItemListPresentationData(presentationData), title: .text(TBLanguage.sharedInstance.localizable(TBLankey.create_group_chain_type)), leftNavigationButton: nil, rightNavigationButton: rightNavigationButton, backNavigationButton: ItemListBackButton(title: presentationData.strings.Common_Back))
