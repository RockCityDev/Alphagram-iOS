...
- (instancetype)initWithTitle:(NSString *)title type:(TGMenuSheetButtonType)type fontSize:(CGFloat)fontSize action:(void (^)(void))action {
    return [self initWithTitle:title type:type fontSize:fontSize icon:nil action:action];
}

- (instancetype)initWithTitle:(NSString *)title type:(TGMenuSheetButtonType)type fontSize:(CGFloat)fontSize icon:(UIImage *)icon action:(void (^)(void))action
{
    ...
    if (self != nil)
    {
        ...
        _imageView.hidden = icon ? false : true;
        _imageView.translatesAutoresizingMaskIntoConstraints = false;
        [self addSubview:_imageView];
        
        [self addConstraints:@[
            [NSLayoutConstraint constraintWithItem:_imageView attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:_button.titleLabel attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:0],
            [NSLayoutConstraint constraintWithItem:_imageView attribute:NSLayoutAttributeLeading relatedBy:NSLayoutRelationEqual toItem:_button.titleLabel attribute:NSLayoutAttributeTrailing multiplier:1.0 constant:10],
            [NSLayoutConstraint constraintWithItem:_imageView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:37],
            [NSLayoutConstraint constraintWithItem:_imageView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:17],
        ]];
