...
public func layoutAuthorizationItemsFromTop(bounds: CGRect,items: [AuthorizationLayoutItem], transition: ContainedViewLayoutTransition) -> CGFloat {
    let solvedItems = items.map { item in
        let aItem = SolvedAuthorizationLayoutItem(item: item)
        aItem.spacingAfter = item.spacingAfter.weight
        aItem.spacingBefore = item.spacingBefore.weight
        return aItem
    }
    
    var verticalOrigin: CGFloat = bounds.minY
    for i in 0 ..< solvedItems.count {
        let item = solvedItems[i]
        verticalOrigin += item.spacingBefore!
        let frame = CGRect(origin: CGPoint(x: floor((bounds.size.width - item.item.size.width) / 2.0), y: verticalOrigin), size: item.item.size)
        if let node = item.item.node {
            transition.updateFrame(node: node, frame: frame)
        } else if let view = item.item.view {
