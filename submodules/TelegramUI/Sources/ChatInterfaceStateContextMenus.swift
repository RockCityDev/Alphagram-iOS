    ...
    |> map { data, updatingMessageMedia, infoSummaryData, appConfig, isMessageRead, messageViewsPrivacyTips, availableReactions, translationSettings, loggingSettings, notificationSoundList -> ContextController.Items in
        ...
        if (!messageText.isEmpty || resourceAvailable || diceEmoji != nil) && !chatPresentationInterfaceState.copyProtectionEnabled && !message.isCopyProtected() {
            ...
            if !isExpired {
                ...
                var canTranslate1 = canTranslate
                ...
                if canTranslate1 {
                    ...
                    actions.append(.action(ContextMenuActionItem(text: TBLanguage.sharedInstance.localizable(TBLankey.translate_setting_title), icon: { theme in
                        return generateTintedImage(image: UIImage(bundleImageName: "Dialog/tb_translate_setting"), color: theme.actionSheet.primaryTextColor)
                    }, action: { _, f in
                        
                        controllerInteraction.performTextSelectionAction(0, NSAttributedString(string: messageText), .tbTranslateSetting)
                        f(.default)
                    })))
