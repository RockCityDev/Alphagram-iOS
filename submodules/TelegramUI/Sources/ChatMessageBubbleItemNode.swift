        ...
        if !messageText.isEmpty || isUnsupportedMedia {
            if !skipText {
                if case .group = item.content, !isFile {
                    ...
                    if !isWalletMessage(str :messageText) {
                        ...
                        result.append((message, TB_ChatMessageWalletBubbleContentNode.self, itemAttributes, BubbleItemAttributes(isAttachment: false, neighborType: .freeform, neighborSpacing: isFile ? .condensed : .default)))
                        needReactions = false
                    }
                }
            } else {
...
class ChatMessageBubbleItemNode: ChatMessageItemView, ChatMessagePreviewItemNode {
    ...
    private var translateButtonNode:ChatMessageTranslateButton?
    
    private var translateBubbleDisposabel : Disposable?
    ...
    deinit {
        self.translateBubbleDisposabel?.dispose()
    }
    ...
    override func didLoad() {
        ...
        recognizer.tapActionAtPoint = { [weak self] point in
            if let strongSelf = self {
                ...
                if let translateButtonNode = strongSelf.translateButtonNode, translateButtonNode.frame.contains(point) && translateButtonNode.alpha != 0 {
                    return.fail
                }
    ...
    private static func beginLayout(selfReference: Weak<ChatMessageBubbleItemNode>, _ item: ChatMessageItem, _ params: ListViewItemLayoutParams, _ mergedTop: ChatMessageMerge, _ mergedBottom: ChatMessageMerge, _ dateHeaderAtBottom: Bool,
        ...
        if  !item.message.text.isEmpty && needTranslate(string:item.message.text) {
            let (can_Translate, _) = canTranslateText(context: item.context, text:   item.message.text, showTranslate: true, ignoredLanguages: nil)
            if can_Translate {
                canTranslate = true
            }
        }
        
        if isWalletMessage(str: item.message.text) {
            canTranslate = false
        }
        ...
        let isRedpack = simpleCheckTransAsset(str: item.message.text)
        
        if initialDisplayHeader && displayAuthorInfo && !isRedpack{
    ...
    private static func applyLayout(selfReference: Weak<ChatMessageBubbleItemNode>,
        ...
        if canTranslate {
            if strongSelf.translateButtonNode == nil {
                let translateButtonNode = ChatMessageTranslateButton()
                strongSelf.translateButtonNode = translateButtonNode
                strongSelf.insertSubnode(translateButtonNode, belowSubnode: strongSelf.messageAccessibilityArea)
                translateButtonNode.addTarget(strongSelf, action: #selector(strongSelf.translateButtonPressed), forControlEvents: .touchUpInside)
            }
        }else if let translateButtonNode = strongSelf.translateButtonNode {
            strongSelf.translateButtonNode = nil
            translateButtonNode.removeFromSupernode()
        }
        
        if let translateButtonNode = strongSelf.translateButtonNode {
            translateButtonNode.alpha = canTranslate ? 1 : 0
        }
        
        if let disposal = strongSelf.translateBubbleDisposabel {
            disposal.dispose()
            strongSelf.translateBubbleDisposabel = nil
        }
        strongSelf.translateBubbleDisposabel = TBAccount.shared.showTranslateBubbleSignal.start(next: {[weak strongSelf] isOn in
            if let translateButtonNode = strongSelf?.translateButtonNode {
                translateButtonNode.alpha = isOn ? 1 : 0
            }
        })
        if case let .System(duration, _) = animation {
            ...
            if let translateButtonNode = strongSelf.translateButtonNode {
                if let shareButtonNode = strongSelf.shareButtonNode  {
                    let currentBackgroundFrame = strongSelf.backgroundNode.frame
                    let buttonSize = translateButtonNode.update(presentationData: item.presentationData, chatLocation: item.chatLocation, subject: item.associatedData.subject, message: item.message, account: item.context.account, disableComments: true)
                    animation.animator.updateFrame(layer: translateButtonNode.layer, frame: CGRect(origin: CGPoint(x: currentBackgroundFrame.maxX + 8.0, y: currentBackgroundFrame.maxY - buttonSize.width - 1.0 - shareButtonNode.frame.size.height - 5.0), size: buttonSize), completion: nil)
                }else{
                    let currentBackgroundFrame = strongSelf.backgroundNode.frame
                    let buttonSize = translateButtonNode.update(presentationData: item.presentationData, chatLocation: item.chatLocation, subject: item.associatedData.subject, message: item.message, account: item.context.account, disableComments: true)
                    animation.animator.updateFrame(layer: translateButtonNode.layer, frame: CGRect(origin: CGPoint(x: currentBackgroundFrame.maxX + 8.0, y: currentBackgroundFrame.maxY - buttonSize.width - 1.0), size: buttonSize), completion: nil)
                }
            }
        } else {
            ...
            if let translateButtonNode = strongSelf.translateButtonNode {
                if let shareButtonNode = strongSelf.shareButtonNode  {
                    let buttonSize = translateButtonNode.update(presentationData: item.presentationData, chatLocation: item.chatLocation, subject: item.associatedData.subject, message: item.message, account: item.context.account, disableComments: true)
                    translateButtonNode.frame = CGRect(origin: CGPoint(x: backgroundFrame.maxX + 8.0 , y: backgroundFrame.maxY - buttonSize.width - 1.0 - shareButtonNode.frame.size.height - 5.0), size: buttonSize)
                }else{
                    let buttonSize = translateButtonNode.update(presentationData: item.presentationData, chatLocation: item.chatLocation, subject: item.associatedData.subject, message: item.message, account: item.context.account, disableComments: true)
                    translateButtonNode.frame = CGRect(origin: CGPoint(x: backgroundFrame.maxX + 8.0, y: backgroundFrame.maxY - buttonSize.width - 1.0), size: buttonSize)
                }
            }
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        if let translateButtonNode = self.translateButtonNode, translateButtonNode.frame.contains(point) && translateButtonNode.alpha != 0 {
            return translateButtonNode.view
        }
    ...
    @objc func translateButtonPressed() {
        if let item = self.item {
            item.controllerInteraction.performTextSelectionAction(0, NSAttributedString(string: item.message.text), .translate)
        }
    }
