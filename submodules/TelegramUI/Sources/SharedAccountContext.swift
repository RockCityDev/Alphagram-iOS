...
public final class SharedAccountContextImpl: SharedAccountContext {
    ...
    private var flyNavManager: TBFlyNavManager?
    ...
    public func makeMyWebPageController(context: AccountContext, peerId: PeerId?, address: String?, isMe: Bool) -> ViewController {
        return TBMyWebController(context: context, peerId: peerId, address: address, isMe: isMe)
    }
    ...
    public func makePeerInfoController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)?, peer: Peer, mode: PeerInfoControllerMode, avatarInitiallyExpanded: Bool, fromChat: Bool, requestsContext: PeerInvitationImportersContext?, fromViewController: ViewController?) -> ViewController? {
        let controller = peerInfoControllerImpl(context: context, updatedPresentationData: updatedPresentationData, peer: peer, mode: mode, avatarInitiallyExpanded: avatarInitiallyExpanded, isOpenedFromChat: fromChat, fromViewController: fromViewController)
   ...
   public func tb_flyStartNav(type: TBFlyMenu, accountContext:AccountContext) {
        let manager = TBFlyNavManager(type: type, accountContext: accountContext)
        self.flyNavManager = manager
        self.flyNavManager?.startNav()
    }
    
    public func tb_tryJoinGroup(params:TBJoinGroupParams) {
        TBJoinGroupManager.tryJoinGroup(params)
    }
    
    public func tb_makeChatController(context: AccountContext, chatLocation: ChatLocation) ->ViewController {
        let controller = ChatControllerImpl(context: context, chatLocation: chatLocation)
        return controller
    }
    ...
    public func makeTBCreateGroupController(context: AccountContext, peerIds: [PeerId], chainName: String?, chainId: String?, initialTitle: String?, mode: CreateGroupMode, completion: ((PeerId, @escaping () -> Void) -> Void)?) -> ViewController {
        return createTBGroupControllerImpl(context: context, peerIds: peerIds, chainName: chainName, chainId: chainId, initialTitle: initialTitle, mode: mode, completion: completion)
    }
    ...
    public func makeToolsCenterController(context: AccountContext) -> ViewController {
        return TBToolsCenterController(context: context)
    }
    ...
    if let _ = peer as? TelegramGroup {
        ...
        case let .group(id):
            ...
            if let vc = fromViewController {
                let peerInfoVc = TBPeerInfoController(context: context, updatedPresentationData: updatedPresentationData, peer: peer, avatarInitiallyExpanded: avatarInitiallyExpanded, isOpenedFromChat: isOpenedFromChat, nearbyPeerDistance: nearbyPeerDistance, callMessages: callMessages, hintGroupInCommon: hintGroupInCommon, fromController: vc)
                vc.present(peerInfoVc,in: .window(.root))
                return nil
            }
