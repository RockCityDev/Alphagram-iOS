...
@objc(AppDelegate) class AppDelegate: UIResponder, UIApplicationDelegate, PKPushRegistryDelegate, UNUserNotificationCenterDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]? = nil) -> Bool {
        ...
        testIsLaunched = true
       let _ = TBAccount.shared.updateSystemCheckSignal().start()
       TBWeb3Config.shared.updateConfig()
        
        
        TBTrack.track(TBTrackEvent.Logging.first_open.rawValue)
    
        let _ = voipTokenPromise.get().start(next: { token in
            self.deviceToken.set(.single(token))
        })
        
        let chainInfo = ParticleNetwork.ChainInfo.ethereum(.goerli)
        ...
        TBLanguage.sharedInstance.setup()
