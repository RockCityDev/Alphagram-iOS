...
final class PeerInfoWalletData {
    let walletAddress: String
    let walletConnect: TBWalletConnect
    
    init(walletAddress: String, walletConnect: TBWalletConnect) {
...
final class PeerInfoScreenData {
    ...
    let walletData: PeerInfoWalletData?
    var limitState: TBGroupLimitState?
    let groupInfo: TBWeb3GroupInfoEntry?
    ...
    init(
        ...
        walletData: PeerInfoWalletData?,
        limitState: TBGroupLimitState? = nil,
        groupInfo: TBWeb3GroupInfoEntry? = nil,
...
func peerInfoScreenSettingsData(context: AccountContext, peerId: EnginePeer.Id, accountsAndPeers: Signal<[(AccountContext, EnginePeer, Int32)], NoError>, activeSessionsContextAndCount: Signal<(ActiveSessionsContext, Int, WebSessionsContext)?, NoError>, notificationExceptions: Signal<NotificationExceptionsList?, NoError>, privacySettings: Signal<AccountPrivacySettings?, NoError>, archivedStickerPacks: Signal<[ArchivedStickerPackItem]?, NoError>, hasPassport: Signal<Bool, NoError>) -> Signal<PeerInfoScreenData, NoError> {
    ...
    let walletSignal = TBWalletConnectManager.shared.availabelConnectionsSignal
    ...
    |> map { peerView, accountsAndPeers, accountSessions, privacySettings, sharedPreferences, notifications, stickerPacks, hasPassport, hasWatchApp, accountPreferences, suggestions, limits, walletConnects -> PeerInfoScreenData in
        ...
        var walletData: PeerInfoWalletData? = nil
        if let walletConect = walletConnects.first, let walletAddress = walletConect.session?.walletInfo?.accounts.first {
            walletData = PeerInfoWalletData(walletAddress: walletAddress, walletConnect: walletConect)
        }
...
func peerInfoScreenData(context: AccountContext, peerId: PeerId, strings: PresentationStrings, dateTimeFormat: PresentationDateTimeFormat, isSettings: Bool, hintGroupInCommon: PeerId?, existingRequestsContext: PeerInvitationImportersContext?, chatLocation: ChatLocation, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>) -> Signal<PeerInfoScreenData, NoError> {
    ...
    |> mapToSignal { inputData -> Signal<PeerInfoScreenData, NoError> in
        ...
        case let .group(groupId):
            ...
            let configSignal:Signal<TBWeb3ConfigEntry?, NoError> = .single(nil) |> then(TBWeb3Config.shared.configSignal)
            
            let groupInfoSignal:Signal<TBWeb3GroupInfoEntry?, NoError> = .single(nil) |> then( TBWeb3GroupInteractor().web3GroupInfoByChatIdSignal(chat_id: String(-abs(groupId.id._internalGetInt64Value()))))
            ...
            |> map { peerView, availablePanes, globalNotificationSettings, status, membersData, currentInvitationsContext, invitations, currentRequestsContext, requests, threadData, preferencesView, config, groupInfo -> PeerInfoScreenData in
                
                var limitState: TBGroupLimitState?
                if let config = config, let groupInfo = groupInfo { 
                    limitState = TBGroupLimitState.transformFromGroupInfo(groupInfo, config: config, peerId: groupId)
                    debugPrint("limitState====\(limitState)")
                }
