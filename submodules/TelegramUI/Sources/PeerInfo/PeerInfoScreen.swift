...
private enum PeerInfoSettingsSection {
    ...
    case tb_tools
    case tb_privacy
    case tb_connect_my_wallet
    case tb_disConnect_my_wallet(connect:TBWalletConnect)
    case tb_edit_nft_avatar
    case emojiStatus
}
...
private final class PeerInfoInteraction {
    ...
    let openTBInviteLinkSetup:(TBGroupLimitState) -> Void
    let openGroupLimit:(TBGroupLimitState) -> Void
    ...
    init(
        ...
        openTBInviteLinkSetup: @escaping (TBGroupLimitState) -> Void,
        openGroupLimit: @escaping (TBGroupLimitState) -> Void,
        ...
        self.openTBInviteLinkSetup = openTBInviteLinkSetup
        self.openGroupLimit = openGroupLimit
...
private func settingsItems(data: PeerInfoScreenData?, context: AccountContext, presentationData: PresentationData, interaction: PeerInfoInteraction, isExpanded: Bool) -> [(AnyHashable, [PeerInfoScreenItem])] {
    ...
    if let walletData = data.walletData {
        
        items[.wallet]!.append(PeerInfoScreenWalletConnectedItem(id: 2, label: .text(PresentationResourcesSettings.tb_icon_connected_wallet_settings!, TBLanguage.sharedInstance.localizable(TBLankey.setting_connected)) , text: walletData.walletAddress, icon: PresentationResourcesSettings.tb_matamask_icon, action: {
            interaction.openSettings(.tb_disConnect_my_wallet(connect: walletData.walletConnect))
        }))
        items[.wallet]!.append(PeerInfoScreenTBDisclosureItem(id: 4, label: .text(TBLanguage.sharedInstance.localizable(TBLankey.setting_go_to_setting), true), text: TBLanguage.sharedInstance.localizable(TBLankey.setting_my_nft_avatar), icon: PresentationResourcesSettings.tb_edit_profile, action: {
            interaction.openSettings(.tb_edit_nft_avatar)
        }))
    }else{
        
        items[.wallet]!.append(PeerInfoScreenTBDisclosureItem(id: 1, label: .text(TBLanguage.sharedInstance.localizable(TBLankey.setting_connect_wallet), true) ,text: TBLanguage.sharedInstance.localizable(TBLankey.setting_my_wallet), icon: PresentationResourcesSettings.tb_my_wallet, action: {
            interaction.openSettings(.tb_connect_my_wallet)
        }))
        
        
        items[.wallet]!.append(PeerInfoScreenTBDisclosureItem(id: 3, text: TBLanguage.sharedInstance.localizable(TBLankey.setting_my_nft_avatar), icon: PresentationResourcesSettings.tb_edit_profile, action: {
            interaction.openSettings(.tb_edit_nft_avatar)
        }))
    }
    
    items[.topgroup]!.append(PeerInfoScreenDisclosureItem(id: 1, text: TBLanguage.sharedInstance.localizable(TBLankey.setting_commontools), icon: PresentationResourcesSettings.tb_tool, action: {
        interaction.openSettings(.tb_tools)
    }))
    
    items[.topgroup]!.append(PeerInfoScreenDisclosureItem(id: 2, text: TBLanguage.sharedInstance.localizable(TBLankey.setting_privacy_setting), icon: PresentationResourcesSettings.tb_privacy, action: {
            interaction.openSettings(.tb_privacy)
    }))
...
private func settingsEditingItems(data: PeerInfoScreenData?, state: PeerInfoState, context: AccountContext, presentationData: PresentationData, interaction: PeerInfoInteraction) -> [(AnyHashable, [PeerInfoScreenItem])] {
    ...
    items[.account]!.append(PeerInfoScreenCommentItem(id: ItemAddAccountHelp, text: TBLanguage.sharedInstance.localizable(TBLankey.settings_AddAnotherAccount_Help)))
...
private func infoItems(data: PeerInfoScreenData?, context: AccountContext, presentationData: PresentationData, interaction: PeerInfoInteraction, nearbyPeerDistance: Int32?, reactionSourceMessageId: MessageId?, callMessages: [Message], chatLocation: ChatLocation) -> [(AnyHashable, [PeerInfoScreenItem])] {
    ...
    if let user = data.peer as? TelegramUser {
        ...
        if let cachedData = data.cachedData as? CachedGroupData {
            ...
            if let limitState = data.limitState {
                items[.tbInvite]!.append(TBPeerInfoScreenButtonItem(id: 1, label: .text("", .systemFont(ofSize: 16, weight: .regular), .white, UIColor(rgb: 0x3954D5)), action: {
                    interaction.openTBInviteLinkSetup(limitState)
                }))
            }
        }
    }
...
private func editingItems(data: PeerInfoScreenData?, context: AccountContext, presentationData: PresentationData, interaction: PeerInfoInteraction) -> [(AnyHashable, [PeerInfoScreenItem])] {
    ...
    if let data = data {
        if let _ = data.peer as? TelegramUser {
            ...
            let ItemUsername = 101
            let ItemTBInvite = 102
            let itemTBLimit = 103
            let ItemInviteLinks = 104
            let ItemPreHistory = 105
            let ItemPermissions = 106
            let ItemAdmins = 107
            let ItemMemberRequests = 108
            let ItemReactions = 109
            
            var canViewAdminsAndBanned = false
            
            
            if let limitState = data.limitState {
                items[.peerTBInvite]!.append(TBPeerInfoScreenButtonItem(id: ItemTBInvite, label: .text("", .systemFont(ofSize: 16, weight: .regular), .white, UIColor(rgb: 0x3954D5)), action: {
                    interaction.openTBInviteLinkSetup(limitState)
                }))
...
final class PeerInfoScreenNode: ViewControllerTracingNode, UIScrollViewDelegate {
    ...
    fileprivate var initialGroupLimit: TBGroupLimitState?
    ...
    init(controller: PeerInfoScreenImpl, context: AccountContext, peerId: PeerId, avatarInitiallyExpanded: Bool, isOpenedFromChat: Bool, nearbyPeerDistance: Int32?, reactionSourceMessageId: MessageId?, callMessages: [Message], isSettings: Bool, hintGroupInCommon: PeerId?, requestsContext: PeerInvitationImportersContext?, chatLocation: ChatLocation, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>) {
        ...
        self._interaction = PeerInfoInteraction(
            ...
            performMemberAction: { [weak self] member, action in
                ...
                if case let .legacyGroupMember(peer, _, _, _) = member, let user = peer.peers[peer.peerId] as? TelegramUser, user.firstName == "Alphagram Group Bot" {
                    strongSelf.controller?.present(standardTextAlertController(theme: AlertControllerTheme(presentationData: strongSelf.presentationData), title: "", text: "，，", actions: [TextAlertAction(type: .genericAction, title: "", action: {
                        
                    }), TextAlertAction(type: .defaultAction, title: "", action: {[weak self] in
                        guard let strongSelf = self else { return }
                        strongSelf.performMemberAction(member: member, action: action)
                    })]), in: .window(.root))
                } else {
                    strongSelf.performMemberAction(member: member, action: action)
                }
            ...
            openTBInviteLinkSetup: { [weak self] limitState in
                self?.editingOpenTBInviteLinkSetup(limitState: limitState)
            },
            openGroupLimit: { [weak self] limitState in
                self?.openGroupLimit(limitState: limitState)
            },
                ...
                if case .done = key {
                    ...
                    if let peer = data.peer as? TelegramUser {
                        ...
                        if title.isEmpty {
                            ...
                            if let initialGroupLimit = self?.initialGroupLimit, let currentGroupLimit = data.limitState, let peerId = data.limitState?.peerId, let groupInfo = data.groupInfo, (initialGroupLimit != currentGroupLimit || (title != groupInfo.title) || (description != groupInfo.description))  { 
                                let uploadEntry = TBGroupLimitState.transform(
                                    id: String(initialGroupLimit.groupId),
                                    chat_id: String(-abs(peerId.id._internalGetInt64Value())),
                                    type: .group,
                                    title: title,
                                    des: description,
                                    ...
                                    avatarData: nil,
                                    tags:groupInfo.tags,
                                    state: currentGroupLimit,
                                    noLimitChainName: nil,
                                    noLimitChainId: nil)
                                #warning("@   ")
        ...
        let _ = TBAccount.shared.nftSettingPromise.get().start(next: {[weak self] a in
            guard let strongSelf = self else { return }
            strongSelf.updateProfilePhoto(a.image) { retList in
                if let profileItem = retList.first,
                    let resource = profileItem.resource as? CloudPeerPhotoSizeMediaResource,
                    let photoId = resource.photoId  {
                    let info = NFTSettingInfoItem(photoId: String(photoId), config: a.config, settingId: a.settingId)
                    let _ = TBAccount.shared.userUpdateNftInfo(info: info).start()
                }
            }
        })
    }
    ...
    private func updateData(_ data: PeerInfoScreenData) {
        ...
        if self.initialGroupLimit == nil && data.limitState != nil {
            self.initialGroupLimit = data.limitState
        }
        ...
        self.context.sharedContext.openResolvedUrl(result, context: self.context, urlContext: .chat(peerId: self.peerId, updatedPresentationData: self.controller?.updatedPresentationData), navigationController: navigationController, forceExternal: false, openPeer: { [weak self] peer, navigation in
            ...
            switch navigation {
                ...
                if let strongSelf = self, peer.restrictionText(platform: "ios", contentSettings: strongSelf.context.currentContentSettings.with { $0 }) == nil {
                    if let infoController = strongSelf.context.sharedContext.makePeerInfoController(context: strongSelf.context, updatedPresentationData: nil, peer: peer._asPeer(), mode: .generic, avatarInitiallyExpanded: false, fromChat: false, requestsContext: nil, fromViewController: nil) {
...
//                        if let strongSelf = self, peer.restrictionText(platform: "ios", contentSettings: strongSelf.context.currentContentSettings.with { $0 }) == nil {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] peer in
            ...
            case .info:
                if peer.restrictionText(platform: "ios", contentSettings: self.context.currentContentSettings.with { $0 }) == nil {
                    if let infoController = self.context.sharedContext.makePeerInfoController(context: self.context, updatedPresentationData: nil, peer: peer._asPeer(), mode: .generic, avatarInitiallyExpanded: false, fromChat: false, requestsContext: nil, fromViewController: nil) {
    ...
    private func editingOpenTBInviteLinkSetup(limitState: TBGroupLimitState) {
        debugPrint("[tb invite]")
        if let peerId = limitState.peerId {
            ProgressHUD.show(interaction:true)
            let _ = (TBWeb3GroupInteractor().web3GroupInfoByChatIdSignal(chat_id: String(-abs(peerId.id._internalGetInt64Value()))) |> deliverOnMainQueue).start {[weak self] groupInfo in
                ProgressHUD.dismiss()
                if let strongSelf = self, let groupInfo = groupInfo {
                    strongSelf.controller?.present(TBInviteLinkListController(context: strongSelf.context, groupInfo: groupInfo, configEntry: limitState.config), in: .window(.root))
                }
            }
        }
    }
    
    private func openGroupLimit(limitState: TBGroupLimitState) {
        let controller = groupLimitControllerImpl(context: context, config: limitState.config, initalLimitState: limitState, update:{[weak self] limitState in
            if let data = self?.data {
                var data = data
    ...
    private func openPeerInfo(peer: Peer, isMember: Bool) {
        ...
        if let infoController = self.context.sharedContext.makePeerInfoController(context: self.context, updatedPresentationData: nil, peer: peer, mode: mode, avatarInitiallyExpanded: false, fromChat: false, requestsContext: nil, fromViewController: nil) {
            (self.controller?.navigationController as? NavigationController)?.pushViewController(infoController)
        }
    }
    ...
    private func deletePeerChat(peer: Peer, globally: Bool) {
        ...
        func popToChat(_ nav: NavigationController?) {
        ...
        for childController in tabController.controllers {
            if let chatListController = childController as? ChatListController {
                chatListController.maybeAskForPeerChatRemoval(peer: EngineRenderedPeer(peer: EnginePeer(peer)), joined: false, deleteGloballyIfPossible: globally, completion: { [weak navigationController] deleted in
                    if deleted {
                        popToChat(navigationController)
                    }
                }, removed: {
                })
                break
    ...
    private func updateProfilePhoto(_ image: UIImage, callBack: (([TelegramMediaImageRepresentation])->Void)? = nil) {
    ...
    private func openAvatarForEditing(fromGallery: Bool = false, completion: @escaping () -> Void = {}) {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] peer, searchBotsConfiguration in
            ...
            let nftMenu = TGMediaAvatarNFTMenu()
            nftMenu.title = TBLanguage.sharedInstance.localizable(TBLankey.setting_use_NFTs_in_your_wallet)
            let mixin = TGMediaAvatarMenuMixin(context: legacyController.context, parentController: emptyController, hasSearchButton: true, hasDeleteButton: hasPhotos && !fromGallery, hasViewButton: false, personalPhoto: strongSelf.isSettings, isVideo: currentIsVideo, saveEditedPhotos: false, saveCapturedMedia: false, signup: false, nftMenu: nftMenu)!
            mixin.stickersContext = paintStickersContext
            let _ = strongSelf.currentAvatarMixin.swap(mixin)
            
            mixin.didChooseNFTMenue = {[weak self] in
                if let strongSelf = self {
                    strongSelf.tryOpenNftListAsAvatar(platform: .metaMask)
                }
            }
    ...
    private func tryOpenNftListAsAvatar(platform:TBWalletConnect.Platform) {
        
       let cs = TBWalletConnectManager.shared.availabelConnections(platform: platform)
        if let c = cs.first {
            self.openNftListAsAvatar(c: c)
        }else{
            
            
            if TBWalletConnectManager.shared.canOpenWalletApp(platform: platform) {
                TBWalletConnectManager.shared.setup(context: self.context)
                TBWalletConnectManager.shared.connectToPlatform(platform: platform) { [weak self] result, c in
                    if let c = c, result {
                        self?.openNftListAsAvatar(c: c)
                    }
                }
            }else{ 
                ProgressHUD.showError(TBLanguage.sharedInstance.localizable(TBLankey.setting_Please_install_the_wallet_before_connecting))
            }
    ...
    private func openNftListAsAvatar(c:TBWalletConnect) {
        let arguments = TBChooseNFTAvatarViewControllerArguments(didChooseOrignalImage:{ image, item in
        ...
        self.controller?.push(TBChooseNFTAvatarViewController(context: self.context, connect: c, arguments: arguments))
    }
    
    
    private func handleClick(c:TBWalletConnect) {
    
        let confirmAction: () -> () = { [weak self] in
            TBWalletConnectManager.shared.disconnect(connect: c)
        }
        let cancel = TextAlertAction(type: .defaultAction, title:self.presentationData.strings.Common_Cancel, action: {})
        let confirm = TextAlertAction(type: .destructiveAction, title:self.presentationData.strings.Common_OK, action: confirmAction)
        let alertVC = textAlertController(context: self.context, title: nil, text:TBLanguage.sharedInstance.localizable(TBLankey.setting_Do_you_want_to_disconnect_the_wallet), actions: [cancel ,confirm])
        self.controller?.present(alertVC, in: .window(.root))
    }
    ...
    fileprivate func openSettings(section: PeerInfoSettingsSection) {
        ...
        switch section {
        case .tb_connect_my_wallet:
            TBTrack.track(TBTrackEvent.Setting.setting_connect_wallet_click.rawValue)
            
            if TBWalletConnectManager.shared.canOpenWalletApp(platform: .metaMask) {
                TBWalletConnectManager.shared.setup(context: self.context)
                TBWalletConnectManager.shared.connectToPlatform(platform: .metaMask)
                TBTrack.track(TBTrackEvent.Wallet.connect_wallet_metamask_click.rawValue)
            }else{
                ProgressHUD.showError(TBLanguage.sharedInstance.localizable(TBLankey.setting_Please_install_the_wallet_before_connecting))
            }
            debugPrint("[TB setting], tb_connect_my_wallet")
        case .tb_disConnect_my_wallet(connect: let connect):

            TBParticleBridge.shared.test()


        case .tb_edit_nft_avatar:
            self.tryOpenNftListAsAvatar(platform: .metaMask)
            debugPrint("[TB setting], tb_edit_nft_avatar")
        case .tb_privacy:
            self.controller?.push(makeTBIncognitoModeViewController(context: self.context))
        case .tb_tools:
            self.controller?.push(self.context.sharedContext.makeToolsCenterController(context: self.context))
        case .avatar:
            ...
            case .addAccount:
                ...
                |> deliverOnMainQueue
                    ...
                    var maximumAvailableAccounts: Int = 20
    ...
    func containerLayoutUpdated(layout: ContainerViewLayout, navigationHeight: CGFloat, transition: ContainedViewLayoutTransition, additive: Bool = false) {
        ...
        if !self.isSettings && !self.state.isEditing {
            ...
            let value = UserDefaults.standard.tb_bool(for: .dontShowMyPhoneOnMyInterface)
            if !value {
                headerHeight += 20
            }
        }
...
public final class PeerInfoScreenImpl: ViewController, PeerInfoScreen, KeyShortcutResponder {
    ...
    public init(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)?, peerId: PeerId, avatarInitiallyExpanded: Bool, isOpenedFromChat: Bool, nearbyPeerDistance: Int32?, reactionSourceMessageId: MessageId?, callMessages: [Message], isSettings: Bool = false, hintGroupInCommon: PeerId? = nil, requestsContext: PeerInvitationImportersContext? = nil, forumTopicThread: ChatReplyThreadMessage? = nil) {
        ...
        let _ = TBWalletConnectManager.shared.availabelConnectionsSignal.start(next: {
            [weak self] cArr in
            debugPrint("[TB, wallet observer]: \(cArr)")
        })
