...
class ChatMessageTranslateButton: HighlightableButtonNode {
    ...
    func update(presentationData: ChatPresentationData, chatLocation: ChatLocation, subject: ChatControllerSubject?, message: Message, account: Account, disableComments: Bool = false) -> CGSize {
        ...
        if self.theme !== presentationData.theme.theme || self.isReplies != isReplies {
            ...
            if message.id.peerId.isRepliesOrSavedMessages(accountPeerId: account.peerId) {
        
                updatedIconOffset = CGPoint(x: UIScreenPixel, y: 1.0)
            }
            self.iconNode.image = UIImage(bundleImageName: "Dialog/tb_translate")
        ...
        if let image = self.iconNode.image {
            self.iconNode.frame = CGRect(origin: CGPoint(x: floor((size.width - image.size.width) / 2.0) + self.iconOffset.x, y: floor((size.width - image.size.width) / 2.0) + self.iconOffset.y), size: image.size)
        }
        return size
    }
}
