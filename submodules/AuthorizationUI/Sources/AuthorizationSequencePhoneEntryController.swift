...
final class AuthorizationSequencePhoneEntryController: ViewController {
    ...
    override public func loadDisplayNode() {
        ...
        if let node = self.displayNode as? AuthorizationSequencePhoneEntryControllerNode {
            node.navigationBar = self.navigationBar
        }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        TBTrack.track(TBTrackEvent.Logging.phone_number_show.rawValue)
        if !self.animatingIn {
            self.controllerNode.activateInput()
        }
    }
    ...
    @objc func nextPressed() {
        TBTrack.track(TBTrackEvent.Logging.phone_number_next.rawValue)
        ...
        if !number.isEmpty {
            ...
            if let (_, id) = existing {
                ...
                if let validLayout = self.validLayout, validLayout.size.width > 320.0 && false {
