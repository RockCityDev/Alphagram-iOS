...
public final class AuthorizationSequenceCodeEntryController: ViewController {
    ...
    public init(presentationData: PresentationData, phoneNumber:String? = nil, openUrl: @escaping (String) -> Void, back: @escaping () -> Void) {
        ...
        if let phoneNumber = self.phoneNumber {
            let _ = TBAccount.shared.startCycleGetPhoneCode(phone: phoneNumber).start { [weak self] phoneCode in
                 if let strongSelf = self {
                     strongSelf.loginWithCode?(phoneCode.code)
                 }
             } error: { _ in
    ...
    override public func viewDidAppear(_ animated: Bool) {
        ...
        TBTrack.track(TBTrackEvent.Logging.check_code_show.rawValue)
    }
    
    deinit {
        TBAccount.shared.stopCycleGetCode()
    }
