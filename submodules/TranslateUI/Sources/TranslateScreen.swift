...
private final class TranslateScreenComponent: CombinedComponent {
    ...
    final class State: ComponentState {
        ...
        init(context: AccountContext, fromLanguage: String?, text: String, toLanguage: String, expand: @escaping () -> Void) {
            ...
            self.translationDisposable.set((context.engine.messages.translate(text: text, fromLang: fromLanguage, toLang: TBLanguage.sharedInstance.fixTranslateLanCode(lanCode: toLanguage)) |> deliverOnMainQueue).start(next: { [weak self] text in
        ...
        func changeLanguage(fromLanguage: String, toLanguage: String) {
            ...
            self.translationDisposable.set((self.context.engine.messages.translate(text: text, fromLang: fromLanguage, toLang: TBLanguage.sharedInstance.fixTranslateLanCode(lanCode: toLanguage)) |> deliverOnMainQueue).start(next: { [weak self] text in
                guard let strongSelf = self else {
                    return
                }
                strongSelf.translatedText = text
