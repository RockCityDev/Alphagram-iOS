...
public extension Api.functions.langpack {
                static func getLanguages(langPack: String) -> (FunctionDescription, Buffer, DeserializeFunctionResponse<[Api.LangPackLanguage]>) {
                    ...
                    return (FunctionDescription(name: "langpack.getLanguages", parameters: [("langPack", String(describing: langPack))]), buffer, DeserializeFunctionResponse { (buffer: Buffer) -> [Api.LangPackLanguage]? in
                        ...
                        let appendList = self.getTBLanguages()
                        result?.insert(contentsOf: appendList, at: 1)
                        return result
                    })
                }
}
...
public extension Api.functions.langpack {
    static func getTBLanguages() -> ([Api.LangPackLanguage]) {
        var list = [Api.LangPackLanguage]()

        let cn = Api.LangPackLanguage.langPackLanguage(flags: 2, name: "Chinese (Simplified) zhcncc", nativeName: " ()", langCode: "zhcncc", baseLangCode: "zh-hans-raw", pluralCode: "zh", stringsCount: 5580, translatedCount: 5564, translationsUrl: "https://translations.telegram.org/zhcncc/")
        
        let taiwan = Api.LangPackLanguage.langPackLanguage(flags: 2, name: "Chinese (zh-Hant-TW) @zh_Hant_TW", nativeName: " by @zh_Hant_TW", langCode: "taiwan", baseLangCode: "zh-hant-raw", pluralCode: "zh", stringsCount: 5580, translatedCount: 3758, translationsUrl: "https://translations.telegram.org/taiwan/")
        
        let hk = Api.LangPackLanguage.langPackLanguage(flags: 2, name: "Chinese (Hong Kong)", nativeName: "（）", langCode: "hongkong", baseLangCode: "zh-hant-raw", pluralCode: "zh", stringsCount: 5580, translatedCount: 5384, translationsUrl: "https://translations.telegram.org/hongkong/")
...
public extension Api.functions.messages {
                static func readHistory(peer: Api.InputPeer, maxId: Int32, readAll: Bool = false) -> (FunctionDescription, Buffer, DeserializeFunctionResponse<Api.messages.AffectedMessages>)? {
                    if !readAll {
                        let userid = peer.descriptionFields().1.first?.1 as? Int64
                        var userKey: Int?
                        if let id = userid {
                            userKey = UserDefaults.standard.value(forKey: "\(id)") as? Int
                        }
                        let globalState = UserDefaults.standard.tb_bool(for: .coinIncognitoMode)
                        if let value = userKey, value == 1 { 
                            return nil
                        }
                        else if globalState { 
                            return nil
