...
public class NavigationBarSearchContentNode: NavigationBarContentNode {
    ...
    public init(theme: PresentationTheme, placeholder: String, compactPlaceholder: String? = nil, inline: Bool = false, textColor: UIColor? = nil, placeholderBackgroundColor: UIColor? = nil, activate: @escaping () -> Void) {
        self.theme = theme
    ...
    private func updatePlaceholder(_ progress: CGFloat, size: CGSize, leftInset: CGFloat, rightInset: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        let textColor = (self.textColor ?? self.theme?.rootController.navigationSearchBar.inputPlaceholderTextColor) ?? UIColor(rgb: 0x8e8e93)
        var fillColor = (self.placeholderBackgroundColor ?? self.theme?.rootController.navigationSearchBar.inputFillColor) ?? .clear
