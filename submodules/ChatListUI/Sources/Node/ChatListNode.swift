...
public final class ChatListNode: ListView {
    ...
    func deleteAll() {
        if let a = self.chatListView?.filteredEntries {
            for item in a {
                if case let .PeerEntry(_, _, _, _, _, _, peer, _, _, _, _, _,_,_, _, _, _, _,_) = item {
                    if let a = peer.peers[peer.peerId] {
                        self.selectedPeer(peer: a)
                    }
                }
            }
        }
    ...
    func selectedPeer(peer: EnginePeer) {
        var didBeginSelecting = false
        var count = 0
        self.updateState { state in
            var state = state
