...
public final class TBQrCodeScanScreen: ViewController {
    public enum Subject {
        case authTransfer(activeSessionsContext: ActiveSessionsContext)
        case peer
        case general(activeSessionsContext:ActiveSessionsContext)
        case wallet
        
        static func transferSubject(subject:TBQrCodeScanScreen.Subject, codes:[CameraCode]) -> TBQrCodeScanScreen.Subject {
            switch subject {
            case .authTransfer, .peer, .wallet:
                return subject
            case let .general(activeSessionsContext):
                let filterCodes = codes.filter { $0.message.hasPrefix("tg://") }
                let walletAddressCodes = codes.filter{$0.message.contains("0x")}
                if filterCodes.count > 0 {
                    return .authTransfer(activeSessionsContext: activeSessionsContext)
                }
                if walletAddressCodes.count > 0 {
                    return .wallet
                }
                return .peer
            }
    ...
    private let subject: TBQrCodeScanScreen.Subject
    ...
    private var controllerNode: TBQrCodeScanScreenNode {
        return self.displayNode as! TBQrCodeScanScreenNode
    }
    ...
    public init(context: AccountContext, subject: TBQrCodeScanScreen.Subject, callBack: ((String) -> Void)? = nil) {
        ...
        self.inForegroundDisposable = (context.sharedContext.applicationBindings.applicationInForeground
            ...
            (strongSelf.displayNode as! TBQrCodeScanScreenNode).updateInForeground(inForeground)
        })
    ...
    private func dismissWithSession(session: RecentAccountSession?) {
        guard let node = self.displayNode as? TBQrCodeScanScreenNode else {
            return
        }
        
        guard let realSubject = node.realSubject else {
            return
        }
        guard case let .authTransfer(activeSessionsContext) = realSubject else {
            return
        }
    ...
    override public func loadDisplayNode() {
        self.displayNode = TBQrCodeScanScreenNode(context: self.context, presentationData: self.presentationData, controller: self, subject: self.subject)
        
        self.displayNodeDidLoad()
        
        self.codeDisposable = ((self.displayNode as! TBQrCodeScanScreenNode).focusedCode.get()
                               |> map { code -> String? in
            return code?.message
        }
    ...
    private func processGeneralResultCode(genealSubject: TBQrCodeScanScreen.Subject, code:String) -> Void {
    ...
    override public func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        (self.displayNode as! TBQrCodeScanScreenNode).containerLayoutUpdated(layout: layout, navigationHeight: self.navigationLayout(layout: layout).navigationFrame.maxY, transition: transition)
    }
}

fileprivate final class TBQrCodeScanScreenNode: ViewControllerTracingNode, UIScrollViewDelegate {
    private let context: AccountContext
    private var presentationData: PresentationData
    private weak var controller: TBQrCodeScanScreen?
    private let subject: TBQrCodeScanScreen.Subject
    var realSubject: (TBQrCodeScanScreen.Subject)?
    ...
    init(context: AccountContext, presentationData: PresentationData, controller: TBQrCodeScanScreen, subject: TBQrCodeScanScreen.Subject) {
        self.context = context
    ...
    override func didLoad() {
        ...
        self.codeDisposable.set((throttledSignal
            ...
            case .peer:
                filteredCodes = codes.filter { $0.message.hasPrefix("https://t.me/") || $0.message.hasPrefix("t.me/") || $0.message.hasPrefix("https://") || $0.message.hasPrefix("http://") }
            case .general:
                filteredCodes = codes.filter { $0.message.hasPrefix("https://t.me/") || $0.message.hasPrefix("t.me/") || $0.message.hasPrefix("tg://") || $0.message.hasPrefix("https://") || $0.message.contains("0x")}
            case .wallet:
                #warning("wallet")
                filteredCodes = codes.filter { $0.message.contains("0x")}
            }
            
            strongSelf.realSubject = TBQrCodeScanScreen.Subject.transferSubject(subject: strongSelf.subject, codes: filteredCodes)
    ...
    func containerLayoutUpdated(layout: ContainerViewLayout, navigationHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        if case .authTransfer = self.subject {
        }else{
    ...
    private func openResolved(_ result: ResolvedUrl) -> Bool {
        ...
        self.context.sharedContext.openResolvedUrl(result, context: self.context, urlContext: .generic, navigationController: navigationController, forceExternal: false, openPeer: { [weak self] peerId, navigation in
            ...
            strongSelf.context.sharedContext.navigateToChatController(NavigateToChatControllerParams(navigationController: navigationController, context: strongSelf.context, chatLocation: .peer(peerId
                                                                                                                                                                                                 ), subject: nil, keepStack: .always, peekData: nil, completion: { [weak navigationController] _ in
                if let navigationController = navigationController {
                    var viewControllers = navigationController.viewControllers
                    viewControllers = viewControllers.filter { controller in
                        if controller is TBQrCodeScanScreen {
                            return false
                        }
                        return true
                    }
