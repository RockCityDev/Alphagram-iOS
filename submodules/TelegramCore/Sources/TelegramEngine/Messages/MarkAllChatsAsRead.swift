                ...
                switch peer {
                    case let .dialogPeer(peer):
                        ...
                        if peerId.namespace == Namespaces.Peer.CloudChannel {
                            ...
                            if let inputPeer = transaction.getPeer(peerId).flatMap(apiInputPeer), let funcData = Api.functions.messages.readHistory(peer: inputPeer, maxId: Int32.max - 1, readAll: true) {
                                signals.append(network.request(funcData)
                                |> map(Optional.init)
