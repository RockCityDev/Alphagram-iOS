    ...
    if peerId.namespace == Namespaces.Peer.SecretChat {
        ...
        |> mapToSignal { inputPeer -> Signal<PeerReadState, PeerReadStateValidationError> in
            ...
            default:
                switch readState {
                case let .idBased(maxIncomingReadId, _, _, _, markedUnread):
                    guard let funcData = Api.functions.messages.readHistory(peer: inputPeer, maxId: maxIncomingReadId) else {
                        return .complete()
                    }
                    var pushSignal: Signal<Void, NoError> = network.request(funcData)
                    |> map(Optional.init)
